...
func AuthMiddleware() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if !user.Active {
			c.AbortWithStatusJSON(401, gin.H{"error": "Akun sudah dinonaktifkan"})
			return
		}
...
// RequireTenantAdmin membatasi pengelolaan akun, nomor, dan konfigurasi kepada
// owner/admin. Akun CS hanya mendapat jalur operasional Inbox yang diizinkan.
...
// CSRouteGuard adalah lapisan backend, bukan sekadar penyembunyian menu.
// Akun CS hanya boleh menggunakan endpoint operasional yang diperlukan.
func CSRouteGuard() gin.HandlerFunc {
	allowed := map[string]map[string]bool{
		"GET": {
			...
			"/api/agents/:id/inbox/events":          true,
			"/api/agents/:id/inbox/incoming-cursor": true,
			"/api/agents/:id/history-sync":          true, "/api/agents/:id/conversation/brief": true,
			"/api/agents/:id/link-preview":          true,
			"/api/agents/:id/crm/contacts": true,
		},
		"POST": {
			"/api/agents/:id/conversation/read": true, "/api/agents/:id/history-sync": true,
			"/api/agents/:id/inbox/client-debug": true,
			"/api/agents/:id/conversation/brief": true, "/api/agents/:id/send": true,
			"/api/agents/:id/send-media": true, "/api/agents/:id/typing": true,
			"/api/agents/:id/crm/contacts": true, "/api/agents/:id/crm/contacts/import": true,
			"/api/agents/:id/crm/contacts/bulk-tag": true, "/api/agents/:id/crm/contacts/bulk-stage": true,
		},
		"PUT": {
			"/api/agents/:id/crm/contacts/:cid": true,
		},
		"DELETE": {
...
func tenantFromToken(tokenStr string) (uint, uint, bool) {
	...
	if tid, ok := claims["tenant_id"].(float64); ok && tid > 0 {
		var agentID uint
		if aid, ok := claims["agent_id"].(float64); ok && aid > 0 {
			agentID = uint(aid)
		}
		return uint(tid), agentID, true
	}
	return 0, 0, false
}
...
func issueMediaToken(tenantID uint, scopedAgentID ...uint) string {
	ttl := time.Duration(config.EnvInt("MEDIA_TOKEN_TTL_MIN", 30)) * time.Minute
	if ttl < time.Minute {
		ttl = 30 * time.Minute
	}
	// Expiry dibulatkan per periode agar polling percakapan menghasilkan token yang
	// sama. Token yang berubah tiap beberapa detik membuat browser memuat ulang gambar.
	expiresAt := time.Now().Truncate(ttl).Add(2 * ttl)
	claims := jwt.MapClaims{
		"tenant_id": tenantID,
		"scope":     "media",
		"exp":       expiresAt.Unix(),
	}
	if len(scopedAgentID) > 0 && scopedAgentID[0] > 0 {
		claims["agent_id"] = scopedAgentID[0]
	}
	token, err := jwt.NewWithClaims(jwt.SigningMethodHS256, claims).SignedString(jwtSecret)
	if err != nil {
...
func Login(c *gin.Context) {
	...
	if !foundUser || !passwordOK || !user.Active {
