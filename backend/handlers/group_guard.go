...
func handleGroupMessage(agentID uint, m services.GroupMessageMeta) {
	// Defense-in-depth: status@broadcast / @newsletter bukan grup sungguhan, tapi
	// whatsmeow menandai broadcast sebagai IsGroup. Jalur live (handleEvent) sudah
	// memfilter, namun jaga agar thread sistem tidak pernah tersimpan lewat jalur
	// manapun yang memanggil handler ini.
	if strings.HasSuffix(m.GroupJID, "@broadcast") || strings.HasSuffix(m.GroupJID, "@newsletter") {
		return
	}
	// Grup masuk ke Inbox sebagai thread manusia, tetapi tidak pernah melewati
	// OnWAMessage sehingga AI, CRM, follow-up, dan webhook personal tetap aman.
	now := m.Timestamp
	if now.IsZero() {
		now = time.Now()
	}
	...
	if m.FromMe {
		...
		text = services.FormatGroupInboxText(text, m.SenderName, m.SenderPN)
	}
	row := models.ChatHistory{
		AgentID: agentID, Sender: m.GroupJID, WAMsgID: m.MessageID,
		DeliveryStatus: "sent", LiveIncoming: !m.FromMe, CreatedAt: now,
	}
	if m.FromMe {
		row.Reply = text
		row.FromHuman = true
	...
	if result.Error != nil {
		log.Printf("[grup] gagal mencatat pesan Inbox agent %d grup %s: %v", agentID, m.GroupJID, result.Error)
		return
	}
	if result.RowsAffected == 0 {
		return
	}
	touchInboxLastMsg(agentID, m.GroupJID, now)
	if m.FromMe {
		return
	}
	publishIncomingInboxEvent(agentID, m.GroupJID, m.MessageID)
	state := models.InboxReadState{AgentID: agentID, Sender: m.GroupJID}
	database.DB.Where("agent_id = ? AND sender = ?", agentID, m.GroupJID).FirstOrCreate(&state)
	if err := database.DB.Model(&state).Updates(map[string]any{
		"whats_app_unread_count": gorm.Expr("whats_app_unread_count + 1"),
		"whats_app_synced":       true,
		"whats_app_state_at":     now,
		"updated_at":             now,
	}).Error; err != nil {
		log.Printf("[grup] gagal memperbarui unread agent %d grup %s: %v", agentID, m.GroupJID, err)
	}
