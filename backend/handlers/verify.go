...
func ForgotPassword(c *gin.Context) {
	...
	services.Go("SendEmail:reset", func() {
		if err := services.SendEmail(user.Email, "Reset Password Ruangkirim", resetHTML); err != nil {
...
func sendVerifyEmail(user models.User) {
	...
	verifyURL := config.Env("APP_URL", "https://ruangkirim.web.id") + "/api/verify-email?token=" + user.EmailVerifyToken
	go func() {
		defer services.RecoverGo("SendEmail:verify")
		if err := services.SendEmail(user.Email, "Verifikasi Email Ruangkirim",
			`<div style="font-family:sans-serif;max-width:480px;margin:0 auto;padding:24px"><h2 style="color:#16a34a">Verifikasi Email</h2><p>Terima kasih sudah mendaftar di Ruangkirim! Klik tombol di bawah untuk mengaktifkan akun kamu:</p><a href="`+verifyURL+`" style="display:inline-block;padding:12px 24px;background:#16a34a;color:#fff;border-radius:8px;text-decoration:none;font-weight:600">Verifikasi Email</a><p style="color:#6b7280;font-size:14px;margin-top:16px">Kalau kamu tidak mendaftar, abaikan email ini.</p></div>`); err != nil {
