...
func main() {
	...
	if wm := license.GetWatermark(); wm.IsBound {
		log.Printf("[license] Source code berlisensi untuk: %s <%s> (Order: %s)", wm.Owner, wm.Email, wm.OrderID)
	}
	...
	// Buang sisa thread sistem WhatsApp (status@broadcast, @newsletter) yang lolos
	// sebelum filter jalur live aktif.
	handlers.CleanupBroadcastJunk()
	// Buang relasi CS→nomor yang yatim (nomor sudah dihapus) agar form CS bisa
	// disimpan dan tidak memunculkan "Nomor X" tanpa checkbox.
	...
	// Sajikan frontend React dari folder dist (bila ada).
	...
	if _, err := os.Stat(staticDir); err == nil {
		...
		r.NoRoute(func(c *gin.Context) {
			// API routes are handled by gin; everything else returns index.html for SPA.
