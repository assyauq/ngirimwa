...
//
// Semua model berjalan lewat OpenRouter. API key dari dashboard menjadi sumber utama;
// OPENROUTER_API_KEY hanya fallback startup untuk instalasi headless.
...
// activePreset selalu menghasilkan konfigurasi OpenRouter yang dipilih di dashboard.
...
// semuanya menggunakan key, model, dan gateway OpenRouter yang sama dengan chat utama.
func CreateAICompletion(ctx context.Context, messages []openai.ChatCompletionMessage, maxTokens int, temperature float32) (openai.ChatCompletionResponse, error) {
	...
	if apiKeyForPreset(p) == "" {
		return openai.ChatCompletionResponse{}, fmt.Errorf("API key OpenRouter belum dikonfigurasi")
	}
...
BATAS RESPONS SAAT INI:
- Policy: %s.
- Maksimal %d kalimat dan sekitar %d karakter.
- Utamakan jawaban inti. Detail tambahan hanya jika diminta pelanggan.
- Jangan menutup dengan tawaran bantuan generik.`, responsePolicy.Name, responsePolicy.MaxSentences, responsePolicy.MaxRunes)
	...
	// Dynamic temperature: presisi saat ada knowledge. Budget token ditentukan oleh
	// response policy sesuai intent agar jawaban WhatsApp tidak melebar.
	temp := float32(0.7)
	maxTok := responsePolicy.MaxTokens
	if len(relevant) > 0 {
		temp = 0.35 // faktual & konsisten menjawab dari knowledge base
	}
	...
	if responseNeedsCondensing(reply, responsePolicy) {
		trace.ResponseRetried = true
		if concise, ok := retryConciseReply(p, messages, responsePolicy); ok {
			reply = concise
		}
	}
	trace.ResponseChars = len([]rune(reply))
	if strings.Contains(reply, "[[ESCALATE]]") {
		return ChatResult{Escalate: true, Model: p.Short, Trace: trace}, nil
	}
	...
	if (len(relevant) > 0 || productContext != "") &&
		...
		if looksFactualUserMessage(userMsg) && !pass {
			...
			if ok {
				...
				if grounded != "" {
					if responseNeedsCondensing(grounded, responsePolicy) {
						log.Printf("WARN: grounding retry melewati response policy %s", responsePolicy.Name)
						grounded = ""
					}
				}
				if grounded != "" {
