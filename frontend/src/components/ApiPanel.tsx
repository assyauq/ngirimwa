...
app.post('/api/kirim-whatsapp', async (req, res) => {
  const response = await fetch('${API_BASE}/messages', {
    method: 'POST',
    headers: {
      Authorization: 'Bearer ' + process.env.RUANGKIRIM_API_KEY,
...
app.post('/webhooks/whatsapp', express.raw({ type: 'application/json' }), (req, res) => {
  ...
  const expected = 'sha256=' + crypto
    .createHmac('sha256', process.env.RUANGKIRIM_WEBHOOK_SECRET)
    .update(rawBody)
    .digest('hex');
...
$expected = 'sha256=' . hash_hmac('sha256', $rawBody, getenv('RUANGKIRIM_WEBHOOK_SECRET'));
    ...
    <Paper variant="outlined" sx={{ p: { xs: 1.25, sm: 2 }, overflow: 'hidden' }}>
      <Stack direction={{ xs: 'column', sm: 'row' }} spacing={0.5} sx={{ mb: 1.5, alignItems: { sm: 'center' }, justifyContent: 'space-between' }}>
        <Box>
          ...
          <Typography variant="caption" color="text.secondary">Data dikirim otomatis saat aktivitas terjadi—sistem Anda tidak perlu mengecek Ruangkirim berulang kali.</Typography>
        </Box>
      ...
      <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ alignItems: 'center', justifyContent: 'center' }}>
        ...
        <WebhookFlowNode icon={<ApiIcon fontSize="small" />} title="Ruangkirim" detail="Menerima aktivitas baru" />
...
export default function ApiPanel({ agentId, onOpenDashboard }: { agentId: number; onOpenDashboard?: () => void }) {
  ...
  const [testText, setTestText] = useState('Uji REST API Ruangkirim — pesan dari dashboard.');
  ...
  return (
    <Box sx={{ maxWidth: 1160 }}>
          ...
          <Accordion disableGutters elevation={0} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: '8px !important', '&:before': { display: 'none' }, overflow: 'hidden' }}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />}>
              <Box>
                ...
                <Typography variant="caption" color="text.secondary">
                  Path <code>/api/kirim-whatsapp</code> ada di <b>server Anda</b>, bukan di Ruangkirim. Server itulah yang memanggil Ruangkirim dengan API key.
                </Typography>
              </Box>
            </AccordionSummary>
            <AccordionDetails sx={{ pt: 0 }}>
              ...
              <Stack direction={{ xs: 'column', md: 'row' }} spacing={1.5}>
                ...
                <Box sx={{ flex: 1, minWidth: 0 }}><CodeBlock label="2. server.js (backend Anda → Ruangkirim)" value={BACKEND_PROXY_EXAMPLE} /></Box>
              </Stack>
              <Typography variant="caption" color="text.secondary" sx={{ display: 'block', mt: 1 }}>
                Simpan key sebagai <code>RUANGKIRIM_API_KEY</code>, jalankan server, buka HTML.
              </Typography>
            </AccordionDetails>
          </Accordion>
      ...
      {tab === 2 && (
        <Stack spacing={1.5}>
          <Alert severity="info">
            <b>Webhook adalah notifikasi otomatis ke server Anda.</b> Saat pesan masuk, gambar selesai dianalisis, atau status berubah, Ruangkirim mengirim HTTP POST berisi JSON ke URL yang disimpan. Halaman HTML biasa tidak dapat menerima webhook; Anda membutuhkan endpoint backend yang dapat diakses melalui HTTPS.
          </Alert>
          ...
          <Paper variant="outlined" sx={{ p: 2 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 1.5 }}>
              Ruangkirim mengirim event pesan masuk, hasil analisis gambar, dan status pengiriman ke URL HTTPS milikmu. Kosongkan URL untuk menonaktifkan.
            </Typography>
          ...
          <Alert severity="info" icon={<ShieldIcon fontSize="inherit" />}>
            Contoh endpoint di atas sudah memverifikasi header X-Signature. Balas HTTP 2xx setelah event diterima; Ruangkirim mencoba ulang setelah 2 dan 5 detik bila endpoint gagal.
          </Alert>
        </Stack>
      )}
    </Box>
