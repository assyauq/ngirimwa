...
import {
  ...
  Accordion, AccordionSummary, AccordionDetails, FormHelperText, Tooltip, ListSubheader,
} from '@mui/material';
...
import {
  useAgents, useAgentStatuses, useAgentStatus, useAgentKnowledge, useAgentUnreadSummary,
...
import type { Agent, KnowledgeItem, AIForm, AIFormStepConfig, AIFormStepType, InboxRealtimeEvent } from '../types';

type AgentAIView = 'overview' | 'persona' | 'knowledge' | 'forms';

function normalizeInboxSenderKey(value?: string) {
  const raw = (value || '').trim();
  if (!raw) return '';
  if (raw.endsWith('@g.us')) return raw;
  return raw.split('@', 1)[0].split(':', 1)[0].replace(/\D/g, '');
}

function InboxRealtimeSubscription({
  agentId,
  activeSender,
  onIncoming,
  onEvent,
  ...
  useInboxRealtime(agentId, activeSender, onIncoming, onEvent, refreshData);
  return null;
}
...
const NAV_GROUPS = [
  ...
  { section: 'Akun', items: [
    { id: 'team', label: 'Pengguna CS', icon: <ManageAccountsOutlinedIcon fontSize="small" /> },
...
export default function Dashboard() {
  ...
  const [tab, setTab] = useState(() => {
    ...
    if (isCS && normalized && !['inbox', 'kontak', 'handoff'].includes(normalized)) return 'inbox';
    return valid && normalized ? normalized : (isCS ? 'inbox' : 'dashboard');
  });
  const [agentAIView, setAgentAIView] = useState<AgentAIView>(() =>
    localStorage.getItem('wai_tab') === 'knowledge' ? 'knowledge' : 'overview');
  // seed = data yang dioper antar-tab (mis. dari Kontak ke Broadcast/Jadwal/Inbox). n = pemicu agar efek jalan ulang.
  const [seed, setSeed] = useState<{ kind: 'broadcast' | 'schedule' | 'inbox' | 'contacts'; value: string; n: number } | null>(null);
  const [agentId, setAgentId] = useState<number>(() => Number(localStorage.getItem('wai_agent')) || 0);
  const [activeInboxSender, setActiveInboxSender] = useState('');
  const [typingPresence, setTypingPresence] = useState<{ agentId: number; sender: string } | null>(null);
  const typingPresenceTimer = useRef(0);
  const [realtimeRevokes, setRealtimeRevokes] = useState<Array<{
    agentId: number;
    sender: string;
    messageId: string;
  }>>([]);
  ...
  const playNotification = useCallback(() => {
    ...
    void playInboxSound().then((played) => {
      if (played) {
        soundWarningShown.current = false;
      } else if (!soundWarningShown.current) {
        soundWarningShown.current = true;
        swalToast('Klik sekali di halaman agar browser mengizinkan suara notifikasi.', 'info');
      }
    }).catch(() => undefined);
  ...
  const handleInboxRealtimeEvent = useCallback((event: InboxRealtimeEvent) => {
    const eventAgentId = Number(event.agent_id) || agentId;
    const sender = normalizeInboxSenderKey(event.sender || event.number || event.chat);
    if (!sender) return;

    if (event.kind === 'typing') {
      if (event.active) {
        if (typingPresenceTimer.current) window.clearTimeout(typingPresenceTimer.current);
        setTypingPresence({ agentId: eventAgentId, sender });
        typingPresenceTimer.current = window.setTimeout(() => {
          setTypingPresence((current) => (
            current?.agentId === eventAgentId && current.sender === sender ? null : current
          ));
          typingPresenceTimer.current = 0;
        }, 8_000);
      } else {
        setTypingPresence((current) => (
          current?.agentId === eventAgentId && current.sender === sender ? null : current
    ...
    if (event.kind === 'incoming') {
      // Pesan yang sudah terkirim otomatis mengakhiri indikator composing,
      // termasuk ketika event paused dari WhatsApp terlambat atau hilang.
      setTypingPresence((current) => (
        current?.agentId === eventAgentId && current.sender === sender ? null : current
      ));
      return;
    ...
    if (event.kind === 'revoke' && messageId) {
      setRealtimeRevokes((current) => {
        if (current.some((item) => (
          item.agentId === eventAgentId && item.sender === sender && item.messageId === messageId
        ))) {
          return current;
        }
        return [...current, { agentId: eventAgentId, sender, messageId }].slice(-512);
      });
    }
  }, [agentId]);
  useEffect(() => () => {
  ...
  const customerTyping = Boolean(
    typingPresence
    && typingPresence.agentId === agentId
    && typingPresence.sender === normalizeInboxSenderKey(activeInboxSender),
  );
  const revokedMessageIDs = useMemo(() => {
    const sender = normalizeInboxSenderKey(activeInboxSender);
    return realtimeRevokes
      .filter((item) => item.agentId === agentId && item.sender === sender)
      .map((item) => item.messageId);
  }, [activeInboxSender, agentId, realtimeRevokes]);
  ...
  const { data: unreadSummary } = useAgentUnreadSummary();
  const { data: statusData } = useAgentStatus(agentId);
  const { data: knowledge = [], refetch: refetchKnowledge } = useAgentKnowledge(agentId, !isCS);
  const activeKnowledge = useMemo(() => knowledge.filter(item => item.active !== false), [knowledge]);
  const personaChecks = useMemo(() => personaQualityChecks(prompt), [prompt]);
  const knowledgeHealth = useMemo(() => knowledgeQuality(activeKnowledge), [activeKnowledge]);
  ...
  const { data: aiForms = [] } = useAIForms(agentId, !isCS);
  const { data: aiFormSubmissions = [] } = useAIFormSubmissions(agentId, !isCS);
  const saveAIFormMut = useSaveAIForm(agentId);
  const deleteAIFormMut = useDeleteAIForm(agentId);

  // ---- Latih dari Website (crawl) ----
  const { data: crawlData } = useCrawlStatus(agentId, !isCS);
  const { data: kbUsage } = useKnowledgeUsage(agentId, !isCS);
  ...
  useEffect(() => {
    if (!notificationSoundEnabled) return undefined;

    const ensureSoundReady = () => {
      void unlockInboxSound()
        .then((ready) => {
          if (ready) soundWarningShown.current = false;
        })
        .catch(() => undefined);
    };
    const recoverVisibleSound = () => {
      if (document.visibilityState === 'visible') ensureSoundReady();
    };

    // Pointer/keyboard menjaga izin audio tetap pulih setelah browser menunda
    // AudioContext. focus/pageshow menutup kasus tab kembali aktif atau laptop
    // bangun dari sleep.
    window.addEventListener('pointerdown', ensureSoundReady, { capture: true, passive: true });
    window.addEventListener('keydown', ensureSoundReady, { capture: true });
    window.addEventListener('focus', ensureSoundReady);
    window.addEventListener('pageshow', ensureSoundReady);
    document.addEventListener('visibilitychange', recoverVisibleSound);
    ensureSoundReady();

    return () => {
      window.removeEventListener('pointerdown', ensureSoundReady, true);
      window.removeEventListener('keydown', ensureSoundReady, true);
      window.removeEventListener('focus', ensureSoundReady);
      window.removeEventListener('pageshow', ensureSoundReady);
      document.removeEventListener('visibilitychange', recoverVisibleSound);
    };
  }, [notificationSoundEnabled]);
  ...
  const toggleNotificationSound = async () => {
    ...
    soundWarningShown.current = false;
    if (!enabled) {
      swalToast('Suara notifikasi dinonaktifkan.', 'info');
      return;
    }
    try {
      const unlocked = await unlockInboxSound();
      if (!unlocked || !await playInboxSound()) {
        swalToast('Browser belum mengizinkan audio. Klik halaman lalu coba lagi.', 'warning');
        return;
      }
      swalToast('Suara notifikasi aktif.', 'success');
    } catch {
      swalToast('Browser belum dapat memutar suara notifikasi.', 'warning');
    }
  };
  ...
  const unreadByAgent = new Map((unreadSummary?.data || []).map(item => [item.agent_id, item.unread_count]));
  const totalUnread = unreadSummary?.total_unread || 0;
  const visibleNavGroups = isCS
    ? NAV_GROUPS
        .map(group => ({ ...group, items: group.items.filter(item => ['inbox', 'kontak', 'handoff'].includes(item.id)) }))
        .filter(group => group.items.length > 0)
    : NAV_GROUPS;
  const filteredAgentOptions = agents.filter(agent => {
    const query = agentSearch.trim().toLowerCase();
    if (!query) return true;
    return `${agent.name || `CS ${agent.id}`} ${agent.number || ''}`.toLowerCase().includes(query);
  });
  ...
  // Nomor terpilih selalu dipantau; nomor lain hanya perlu stream saat benar-benar
  // tersambung agar banyak akun offline tidak menambah polling di Dashboard.
  const notificationAgentIds = Array.from(new Set([
    agentId,
    ...agents
      .filter((agent) => statusMap[agent.id] === 'connected')
      .map((agent) => agent.id),
  ]))
    .filter((id) => id > 0);
  const setupIssues = [
    (activeKnowledge.length > 0 || prompt.trim() !== '') ? '' : 'Sebelum mengaktifkan Balasan AI, lengkapi Persona atau Pengetahuan aktif di menu Asisten AI.',
  ].filter(Boolean);
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, minHeight: '100vh', height: { md: '100vh' }, overflow: { md: 'hidden' }, bgcolor: 'background.default' }}>
      {notificationAgentIds.map((notificationAgentId) => (
        <InboxRealtimeSubscription
          key={notificationAgentId}
          agentId={notificationAgentId}
          activeSender={notificationAgentId === agentId ? activeInboxSender : ''}
          onIncoming={handleIncomingMessage}
          onEvent={handleInboxRealtimeEvent}
          refreshData={notificationAgentId === agentId}
        />
      ))}
      <Box
        component="aside"
        sx={{
          width: { xs: '100%', md: 'var(--sidebar-width, 248px)' },
          flexShrink: 0,
          p: { xs: 1, md: 1.5 },
          display: 'flex',
      ...
      >
        <Stack direction={{ xs: 'row', md: 'column' }} spacing={1.25} sx={{ alignItems: { xs: 'center', md: 'stretch' }, minHeight: { xs: 44, md: 'auto' } }}>
          ...
          {!isCS && <Box sx={{ width: '100%', flexShrink: 0, display: { xs: 'none', md: 'block' } }}>
            <Button fullWidth variant="outlined" startIcon={<AddIcon />} onClick={openAddAgent} disabled={createAgentMut.isPending}>
              Tambah CS
            </Button>
          </Box>}
          <Tooltip title={notificationSoundEnabled ? 'Suara pesan aktif' : 'Suara pesan nonaktif'}>
            <IconButton
              aria-label={notificationSoundEnabled ? 'Nonaktifkan suara pesan' : 'Aktifkan suara pesan'}
          ...
          <IconButton aria-label="Logout" onClick={logout} color="error" sx={{ display: { xs: 'inline-flex', md: 'none' } }}>
        ...
        <Box
          sx={{
            ...
            inset: { xs: '61px 0 0 0', md: 'auto' },
            px: { xs: 1.5, md: 0 },
            py: { xs: 1.5, md: 0 },
            bgcolor: 'background.paper',
            borderTop: { xs: '1px solid', md: 0 },
            borderColor: 'divider',
            zIndex: { xs: 119, md: 'auto' },
          }}
        >
          {visibleNavGroups.map((group, gi) => (
            <Fragment key={group.section || 'main'}>
              {group.section && (
                <Typography
                  ...
                  sx={{
                    ...
                    fontWeight: 700, fontSize: '0.68rem', letterSpacing: '0.055em',
                ...
                return (
                  ...
                  >
                    {item.id === 'inbox' && totalUnread > 0 ? (
                      <Badge badgeContent={totalUnread > 99 ? '99+' : totalUnread} color="error" sx={{ mr: 0.5 }}>
                        {item.label}
                      </Badge>
              ...
              {!isCS && gi === visibleNavGroups.length - 1 && (
                <Stack spacing={0.75} sx={{ display: { xs: 'flex', md: 'none' }, mt: 2, pt: 1.5, borderTop: '1px solid', borderColor: 'divider' }}>
                  <Button fullWidth variant="contained" startIcon={<AddIcon />} onClick={() => { setSidebarOpen(false); openAddAgent(); }}>
                    Tambah CS
                  </Button>
                  <Button fullWidth variant="outlined" startIcon={<ManageAccountsOutlinedIcon />} onClick={() => { setSidebarOpen(false); setManageOpen(true); }}>
                    Kelola CS
                  </Button>
                </Stack>
              )}
        ...
        <Stack spacing={0.35} sx={{ display: { xs: 'none', md: 'flex' }, pt: 1, borderTop: '1px solid', borderColor: 'divider' }}>
          <Button
            startIcon={notificationSoundEnabled ? <VolumeUpOutlinedIcon /> : <VolumeOffOutlinedIcon />}
            onClick={() => void toggleNotificationSound()}
            color={notificationSoundEnabled ? 'primary' : 'inherit'}
            sx={{ justifyContent: 'flex-start', fontWeight: 600, height: 36, px: 1.5, borderRadius: 1 }}
          >
            Suara pesan {notificationSoundEnabled ? 'aktif' : 'nonaktif'}
          </Button>
          <Button
            startIcon={<PersonIcon />}
            onClick={() => setProfileModalOpen(true)}
            sx={{ justifyContent: 'flex-start', color: 'text.secondary', fontWeight: 600, height: 36, px: 1.5, borderRadius: 1 }}
          >
            Profil
          </Button>
          <Button
      ...
      <Box
        ...
        sx={{
          ...
          p: tab === 'inbox' ? 0 : { xs: 1.5, md: 3 },
          overflowY: tab === 'inbox' ? 'hidden' : 'auto',
          height: { md: '100vh' },
          minHeight: 0,
          width: '100%',
          maxWidth: tab === 'inbox' ? 'none' : '1280px',
          mx: tab === 'inbox' ? 0 : 'auto',
          minWidth: 0,
      ...
      >
        {agents.length > 0 && tab !== 'inbox' && (
          <Box
            sx={{
              ...
              gap: 1,
              px: { xs: 1.25, md: 1.5 },
              mx: tab === 'inbox' ? { xs: 1, md: 1.5 } : 0,
              mt: tab === 'inbox' ? { xs: 0.75, md: 1 } : 0,
              py: 1,
              mb: tab === 'inbox' ? 0.75 : 1.5,
              bgcolor: 'background.paper',
              border: '1px solid',
              borderColor: 'divider',
              borderRadius: 1.25,
          ...
          >
            <Stack direction="row" spacing={1} sx={{ alignItems: 'center', minWidth: 0 }}>
              <Box sx={{ width: 9, height: 9, borderRadius: '50%', bgcolor: dotColor(statusMap[agentId]), flexShrink: 0 }} />
              <Box sx={{ minWidth: 0, display: { xs: 'none', sm: 'block' } }}>
                <Typography sx={{ fontSize: 11.5, fontWeight: 700, color: 'text.secondary', lineHeight: 1.2 }}>
                  Customer Service aktif
                </Typography>
                <Typography noWrap sx={{ fontSize: 13.5, fontWeight: 700, lineHeight: 1.35 }}>
                  {currentAgent?.name || `CS ${agentId}`}
                </Typography>
              </Box>
              <FormControl size="small" sx={{ minWidth: { xs: 152, sm: 210 } }}>
                <Select
                  value={agentId}
                  aria-label="Pilih Customer Service"
                  onChange={event => setAgentId(Number(event.target.value))}
                  onOpen={() => setAgentSearch('')}
                  renderValue={() => (
                    <Stack direction="row" spacing={1} sx={{ alignItems: 'center', minWidth: 0 }}>
                      <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: dotColor(statusMap[agentId]), flexShrink: 0 }} />
                      <Typography noWrap sx={{ fontSize: 13.5, fontWeight: 600 }}>
                        {currentAgent?.name || `CS ${agentId}`}
                      </Typography>
                      {(unreadByAgent.get(agentId) || 0) > 0 && (
                        <Chip size="small" color="error" label={unreadByAgent.get(agentId)} sx={{ height: 20, minWidth: 24 }} />
                      )}
                    </Stack>
                  )}
                  MenuProps={{
                    ...
                    slotProps: {
                      list: { sx: { pt: 0, maxHeight: 360 } },
                    },
                  }}
                  sx={{ height: 36, bgcolor: 'background.paper', '& .MuiSelect-select': { py: 0.75 } }}
                >
                  <ListSubheader sx={{ pt: 1, pb: 0.75, bgcolor: 'background.paper' }}>
                    <TextField
                      autoFocus
                      fullWidth
                      size="small"
                      value={agentSearch}
                      placeholder="Cari nama atau nomor CS"
                      onChange={event => setAgentSearch(event.target.value)}
                      onKeyDown={event => {
                        if (event.key !== 'Escape') event.stopPropagation();
                      }}
                      onClick={event => event.stopPropagation()}
                      slotProps={{ htmlInput: { 'aria-label': 'Cari Customer Service' } }}
                    />
                  </ListSubheader>
                  {filteredAgentOptions.map(agent => (
                    <MenuItem key={agent.id} value={agent.id}>
                      <Box component="span" sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: dotColor(statusMap[agent.id]), mr: 1, flexShrink: 0 }} />
                      <Box sx={{ minWidth: 0 }}>
                        <Typography noWrap sx={{ fontSize: 13, fontWeight: 600 }}>{agent.name || `CS ${agent.id}`}</Typography>
                        {agent.number && <Typography noWrap variant="caption" color="text.secondary">+{agent.number}</Typography>}
                      </Box>
                      {(unreadByAgent.get(agent.id) || 0) > 0 && (
                        <Chip size="small" color="error" label={unreadByAgent.get(agent.id)} sx={{ ml: 'auto', height: 21, minWidth: 25 }} />
                      )}
                    </MenuItem>
                  ))}
                  {filteredAgentOptions.length === 0 && (
                    <MenuItem disabled>Customer Service tidak ditemukan</MenuItem>
                  )}
                </Select>
              </FormControl>
            </Stack>
            <Stack direction="row" spacing={0.75} sx={{ alignItems: 'center', flexShrink: 0 }}>
              ...
              />
              {!isCS && <Tooltip title="Kelola Customer Service">
                <IconButton onClick={() => setManageOpen(true)} sx={{ width: 34, height: 34, border: '1px solid', borderColor: 'divider' }}>
        ...
        {tab === 'dashboard' && (
          <Box>
            <PageHeader
              ...
              subtitle="Pantau kondisi layanan dan hal yang membutuhkan perhatian."
            />
            ...
            <Grid container spacing={2} sx={{ mb: 2 }}>
                ...
                <Card sx={{ height: '100%' }}>
                  <CardContent>
                    <Stack direction="row" spacing={1} sx={{ alignItems: 'center', justifyContent: 'space-between', mb: 1.75 }}>
                      <Box>
                        <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Kondisi operasional</Typography>
                        <Typography variant="caption" color="text.secondary">Ringkasan nomor dan kesiapan layanan saat ini.</Typography>
                      </Box>
                      ...
                      {[
                        {
                          label: 'WhatsApp',
                          value: sl,
                          detail: status === 'connected' ? 'Siap menerima pesan' : 'Perlu ditautkan',
                          icon: <QrCodeIcon fontSize="small" />,
                          color: status === 'connected' ? 'success.main' : 'warning.main',
                        },
                        {
                          label: 'CS online',
                          value: `${connectedCS}/${agents.length}`,
                          detail: 'Nomor sedang tersambung',
                          icon: <SupportAgentIcon fontSize="small" />,
                          color: connectedCS > 0 ? 'success.main' : 'text.secondary',
                        },
                        {
                          label: 'Butuh CS',
                          value: String(handoffs.length),
                          detail: handoffs.length > 0 ? 'Percakapan perlu ditangani' : 'Tidak ada antrean',
                          icon: <SupportAgentIcon fontSize="small" />,
                          color: handoffs.length > 0 ? 'warning.main' : 'success.main',
                          onClick: () => setTab('handoff'),
                        },
                        {
                          label: 'Pengetahuan AI',
                          value: String(knowledge.length),
                          detail: knowledge.length > 0 ? 'Sumber jawaban tersedia' : 'Belum ada sumber',
                          icon: <KnowledgeIcon fontSize="small" />,
                          color: knowledge.length > 0 ? 'primary.main' : 'warning.main',
                          onClick: () => openAgentAI('knowledge'),
                        },
                      ].map(item => (
                        <Grid key={item.label} size={{ xs: 12, sm: 6 }}>
                          ...
                          >
                            <Stack direction="row" spacing={1.25} sx={{ alignItems: 'flex-start' }}>
                              <Box sx={{ width: 34, height: 34, borderRadius: 1, bgcolor: 'background.default', color: item.color, display: 'grid', placeItems: 'center', flexShrink: 0 }}>
                                {item.icon}
                              </Box>
                              <Box sx={{ minWidth: 0 }}>
                                <Typography variant="caption" color="text.secondary" sx={{ fontWeight: 700 }}>{item.label}</Typography>
                                <Typography sx={{ fontWeight: 700, fontSize: 20, lineHeight: 1.25 }}>{item.value}</Typography>
                    ...
                    {status === 'connected' && (
                      <Stack direction="row" spacing={1} sx={{ mt: 1.5, justifyContent: 'flex-end' }}>
                        <Button variant="text" size="small" onClick={connect} disabled={connectMut.isPending} startIcon={<QrCodeIcon />}>
                          Sambungkan ulang
                        </Button>
                        <Button variant="text" size="small" color="error" onClick={disconnectWA} disabled={disconnectMut.isPending}>
                          Putuskan
                        </Button>
                      </Stack>
                    )}
              ...
              <Grid size={{ xs: 12, lg: 4 }}>
                <Card sx={{ height: '100%' }}>
                  <CardContent>
                    <Typography variant="subtitle1" sx={{ fontWeight: 700 }}>Kontrol nomor</Typography>
                    <Typography variant="caption" color="text.secondary">Pengaturan operasional yang paling sering digunakan.</Typography>

                    <Stack spacing={0} sx={{ mt: 1.5 }}>
                      <Stack direction="row" spacing={1.25} sx={{ alignItems: 'center', justifyContent: 'space-between', py: 1.25 }}>
                        <Stack direction="row" spacing={1} sx={{ alignItems: 'flex-start', minWidth: 0 }}>
                          <SmartToyIcon fontSize="small" color={aiEnabled ? 'success' : 'disabled'} />
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 700 }}>Balasan AI</Typography>
                            <Typography variant="caption" color="text.secondary">
                              {aiEnabled ? 'Aktif membalas pelanggan' : 'Balasan dilakukan manual'}
                            </Typography>
                          </Box>
                        </Stack>
                        <Switch checked={aiEnabled} onChange={event => toggleAI(event.target.checked)} color="success" disabled={!agentId || saveAgentMut.isPending} />
                      </Stack>
                      ...
                      <Stack direction="row" spacing={1.25} sx={{ alignItems: 'center', justifyContent: 'space-between', py: 1.25 }}>
                        <Stack direction="row" spacing={1} sx={{ alignItems: 'flex-start', minWidth: 0 }}>
                          <MarkChatReadIcon fontSize="small" color={autoRead ? 'success' : 'disabled'} />
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 700 }}>Tanda dibaca</Typography>
                            <Typography variant="caption" color="text.secondary">
                              {autoRead ? 'Dibaca saat pesan masuk' : 'Dibaca saat akan dibalas'}
                            </Typography>
                          </Box>
                        </Stack>
                        <Switch checked={autoRead} onChange={event => { void toggleAutoRead(event.target.checked); }} disabled={!agentId || saveAgentMut.isPending} />
                      </Stack>
                    </Stack>

                    <Button fullWidth variant="outlined" onClick={() => openAgentAI('overview')} startIcon={<SmartToyIcon />} sx={{ mt: 1 }}>
                      Kelola Asisten AI
                    </Button>
                  </CardContent>
                </Card>
            ...
            {(handoffs.length > 0 || setupIssues.length > 0) && (
              <Card>
                <CardContent>
                  <Typography variant="subtitle1" sx={{ fontWeight: 700, mb: 1.25 }}>Perlu perhatian</Typography>
                  <Stack spacing={1}>
                    {handoffs.length > 0 && (
                      <Paper variant="outlined" sx={{ p: 1.25, borderRadius: 1.25 }}>
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between' }}>
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 700 }}>{handoffs.length} percakapan menunggu Customer Service</Typography>
                            <Typography variant="caption" color="text.secondary">Tinjau percakapan yang tidak dapat diselesaikan otomatis.</Typography>
                          </Box>
                          <Button size="small" variant="contained" onClick={() => setTab('handoff')}>Buka antrean</Button>
                        </Stack>
                      </Paper>
                    )}
                    {setupIssues.length > 0 && (
                      <Paper variant="outlined" sx={{ p: 1.25, borderRadius: 1.25 }}>
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ alignItems: { sm: 'center' }, justifyContent: 'space-between' }}>
                          <Box>
                            <Typography variant="body2" sx={{ fontWeight: 700 }}>Asisten AI belum siap sepenuhnya</Typography>
                            <Typography variant="caption" color="text.secondary">{setupIssues[0]}</Typography>
                          </Box>
                          <Button size="small" variant="outlined" onClick={() => openAgentAI('overview')}>Lengkapi sekarang</Button>
                        </Stack>
                      </Paper>
                    )}
                  </Stack>
          ...
          <Box>
            <Paper variant="outlined" sx={{ p: 1.25, mb: 1.25 }}>
              <Stack direction={{ xs: 'column', sm: 'row' }} sx={{ gap: 1, alignItems: { xs: 'stretch', sm: 'flex-start' }, justifyContent: 'space-between' }}>
                ...
                <Chip
                  size="small"
                  color={activeKnowledge.length ? 'success' : 'default'}
                  label={`${activeKnowledge.length} aktif${knowledge.length > activeKnowledge.length ? ` · ${knowledge.length - activeKnowledge.length} nonaktif` : ''}`}
                />
              </Stack>
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'repeat(2, minmax(0, 1fr))', sm: 'repeat(4, minmax(0, 1fr))' }, gap: 0.75, mt: 1 }}>
                {[
                  { label: 'Memiliki tag', value: `${knowledgeHealth.tagged}/${activeKnowledge.length}` },
                  { label: 'Jawaban jelas', value: `${knowledgeHealth.detailed}/${activeKnowledge.length}` },
                  { label: 'Pencarian AI', value: kbUsage?.semantic_search ? `Semantik ${kbUsage.embedded_knowledge || 0}/${activeKnowledge.length}` : 'Kata kunci' },
        ...
        {tab === 'inbox' && (
          <Box sx={{ flex: 1, minHeight: 0, height: { xs: 'calc(100dvh - 56px)', md: '100%' }, display: 'flex', flexDirection: 'column' }}>
            {agents.length > 0 && (
              ...
              >
                <Stack direction="row" spacing={0.85} sx={{ alignItems: 'center', minWidth: 0 }}>
                  <Box sx={{
                    width: 32, height: 32, borderRadius: 1.1, display: 'grid', placeItems: 'center',
                  ...
                  <Box sx={{ minWidth: 0 }}>
                    <Stack direction="row" spacing={0.65} sx={{ alignItems: 'center' }}>
                      <Typography sx={{ fontSize: 13, fontWeight: 750, lineHeight: 1.2, whiteSpace: 'nowrap' }}>
                        Pantau Inbox
                      </Typography>
                      {totalUnread > 0 && (
                        <Box component="span" sx={{
                          px: 0.65, minWidth: 22, height: 19, borderRadius: 10,
                          display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                          bgcolor: 'error.main', color: 'error.contrastText', fontSize: 10.5, fontWeight: 800,
                        }}>
                          {totalUnread > 999 ? '999+' : totalUnread}
                        </Box>
                      )}
                    </Stack>
                    <Typography variant="caption" color="text.secondary" sx={{ display: 'block', lineHeight: 1.25, whiteSpace: 'nowrap' }}>
                      {connectedCS} online · {agents.length} nomor dipantau
                    </Typography>
                  </Box>
                </Stack>
                    ...
                    return (
                      ...
                      >
                        <Box component="span" sx={{
                          ...
                          bgcolor: dotColor(statusMap[agent.id]),
                          boxShadow: selected ? '0 0 0 2px rgba(255,255,255,.28)' : 'none',
                        }} />
                        <Typography component="span" sx={{ fontSize: 12.5, fontWeight: selected ? 750 : 650 }}>
                          {agent.name || `CS ${agent.id}`}
                        </Typography>
                        {unread > 0 && (
                          <Box component="span" sx={{
                            px: 0.55, minWidth: 20, height: 18, borderRadius: 9,
                            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                            bgcolor: selected ? 'common.white' : 'error.main',
                            color: selected ? 'primary.main' : 'error.contrastText',
                ...
                <Stack direction="row" spacing={0.6} sx={{ alignItems: 'center', justifyContent: { xs: 'space-between', md: 'flex-end' } }}>
                  <Chip
                    size="small"
                    label={`${currentAgent?.name || `CS ${agentId}`} · ${sl}`}
                    color={sc}
                    variant="outlined"
                    sx={{ maxWidth: 190, '& .MuiChip-label': { overflow: 'hidden', textOverflow: 'ellipsis' } }}
                  />
                  {!isCS && (
                    <Tooltip title="Kelola Customer Service">
                      <IconButton
                        onClick={() => setManageOpen(true)}
                        size="small"
                        aria-label="Kelola Customer Service"
                        sx={{ width: 31, height: 31, border: '1px solid', borderColor: 'divider', borderRadius: 1 }}
                      >
            ...
            <InboxPanel
              ...
              onToggleNotificationSound={() => void toggleNotificationSound()}
              onActiveSenderChange={setActiveInboxSender}
              customerTyping={customerTyping}
              revokedMessageIDs={revokedMessageIDs}
              seed={seed?.kind === 'inbox' ? seed : null}
            />
          </Box>
        )}
        {tab === 'coba-chat' && <TestChatPanel key={agentId} agentId={agentId} />}
        {tab === 'grup' && <GroupGuardPanel agentId={agentId} />}
        {tab === 'broadcast' && <BroadcastPanel agentId={agentId} seed={seed?.kind === 'broadcast' ? seed : null} />}
        {tab === 'kalender' && (
          <CalendarPanel
            agentId={agentId}
            seed={seed?.kind === 'schedule' ? seed : null}
            onSeedConsumed={() => setSeed(prev => (prev?.kind === 'schedule' ? null : prev))}
          />
        )}
        ...
        {tab === 'team' && !isCS && <TeamPanel />}
        {tab === 'kontak' && (
          <ContactsPanel
            key={`${agentId}:${seed?.kind === 'contacts' ? seed.n : 0}`}
            agentId={agentId}
            canBroadcast={!isCS}
            onBroadcast={(recipients) => { setSeed({ kind: 'broadcast', value: recipients, n: Date.now() }); setTab('broadcast'); }}
            onScheduleBroadcast={(recipients) => { setSeed({ kind: 'schedule', value: recipients, n: Date.now() }); setTab('kalender'); }}
          ...
          <Stack spacing={1.25} sx={{ pt: 0.5 }}>
            ...
            <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ alignItems: { sm: 'center' } }}>
              ...
              <TextField
                ...
                type="number"
                label="Prioritas"
                value={editingKnowledgeDraft.priority}
                onChange={e => setEditingKnowledgeDraft(d => ({ ...d, priority: Math.max(-100, Math.min(100, Number(e.target.value) || 0)) }))}
                helperText="-100 sampai 100"
                sx={{ width: { sm: 150 } }}
              />
            </Stack>
          </Stack>
